...
PHP_VERSION=`curl -s https://www.php.net/downloads.php?source=Y | grep "PHP $PHP_BRANCH\." | grep Changelog | grep -v logo | head -n 1 | $SED -r -e 's/^.*PHP +//;s/<.*>//;s/ +//g;s/\(.*\)//'`
echo "$PHP_VERSION"
